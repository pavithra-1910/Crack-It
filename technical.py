...
# Fetching topics from Firebase
ref = db.reference('technical_questions')
topics = ref.get()
...
class TechnicalQuestionsScreen(Screen):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)

        # Set background color using Canvas
        with self.canvas.before:
            Color(0, 0, 1, 0.8)  # Blue with full opacity
    ...
    def back_button(self, instance):
        self.manager.current = "interview_section"  # Update with actual previous screen name

    def show_topic(self, instance):
        if len(topics) > 0:
            # Get next topic
            topic_name, topic_content = topics.popitem()
            self.topic_label.text = f"Topic: {topic_content['title']}"
            # Creating bullet points with gaps between them
            bullet_points = "\n\n".join([f"- {point}" for point in topic_content['points']])
            self.bullet_point_label.text = bullet_points
        else:
            self.topic_label.text = "No more topics!"
